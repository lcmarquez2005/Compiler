...
public class LexicoGUI extends JFrame {
    ...
    private void createUI() {
        ...
        // Panel superior con info del alumno y docente
        ...
        inputArea.setText("2+5=0");

        JScrollPane inputScroll = new JScrollPane(inputArea);
        inputPanel.add(inputScroll, BorderLayout.CENTER);

        // Tabla de resultados
        String[] columnNames = {"Lexema", "Token", "Tipo"};
        ...
        headerRenderer.setBackground(new Color(230, 230, 250)); // lavanda
        ...
        analyzeBtn.addActionListener(this::analyzeCode);
    ...
    private void analyzeCode(ActionEvent e) {
        ...
        List<LexicoCore.Token> tokens = LexicoCore.analizarExpresion(code);
        for (LexicoCore.Token token : tokens) {
            String tokenType = switch (token.tipo) {
                case LexicoCore.NUMERO -> "NUMERO_ENTERO";
                case LexicoCore.OPERADOR -> token.lexema.equals("=") ? "OP_ASIGN" : "OP_" + token.lexema;
                case LexicoCore.PARENTESIS -> "PAR_" + token.lexema;
                default -> "DESCONOCIDO";
            };

            String lexType = switch (token.tipo) {
                case LexicoCore.NUMERO -> "Literal Numérico";
                case LexicoCore.OPERADOR -> "Operador";
                case LexicoCore.PARENTESIS -> "Paréntesis";
                default -> "No reconocido";
            };

            tableModel.addRow(new Object[]{token.lexema, tokenType, lexType});
        }
    }
